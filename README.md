...
출력된 64자리 문자열을 `.env` 의 `MFOAM_PROV_KEY=` 뒤에 붙여넣고 저장한다. `.env` 는 공유하지 않는다.
...
cd rateCV
cp .env.example .env
python3 -c "import secrets;print(secrets.token_hex(32))"   # 출력값을 .env 에 붙여넣기
```
...
| `T_C` | 물 온도 (°C) |
| `start`, `end` | (선택) 영상 안의 구간, 초 또는 `"m:ss"` |

한 영상에 여러 실험을 이어 찍었으면 `stem` 을 같게 적는다. 적는 순서가 영상 순서다.
`start`/`end` 가 없으면 붓는 순간을 찾아 자동으로 나눈다.
찾은 실험 수가 클립 수와 다르면 멈추니 그때 `start`/`end` 를 적는다.

```json
    "cold": {"stem": "20260824_150000", "T_C": 14.2, "start": "0:00",  "end": "5:20"},
    "rt":   {"stem": "20260824_150000", "T_C": 27.2, "start": "5:30",  "end": "10:10"},
    "hot":  {"stem": "20260824_150000", "T_C": 54.7, "start": "10:20", "end": "11:40"}
```
...
단계를 따로 돌리려면 `.venv\Scripts\python src\extract.py` 처럼 `src\` 의 파일을 하나씩 부른다.
`src\repro.py` 는 measure + analyze 를 두 번 돌려 결과가 바이트 단위로 같은지 본다.
...
| `실험을 n개 찾았다 (clips.json 은 3개)` | 실험 사이에 관을 비우고 20 초 이상 두었는지 본다. 안 되면 `start`/`end` 를 적는다 |
| `"start"/"end" 는 전부 적거나 전부 빼라` | 한 영상을 쓰는 클립은 셋 다 적거나 셋 다 뺀다 |
