...
`실험을 n개 찾았다 (clips.json 은 3개)` 가 뜨면 자동으로 못 나눈 것이다. 각 클립에 구간을 분:초로 직접 적는다:
...
구간은 붓기 몇 초 전부터 잡으면 된다. 시작점은 프로그램이 거품이 솟는 프레임으로 다시 잡는다.
...
| `실험을 n개 찾았다 (clips.json 은 3개)` | 이어 찍은 영상을 자동으로 못 나눴다. 3번의 `start`/`end` 를 직접 적는다 |
| `"start"/"end" 는 전부 적거나 전부 빼라` | 한 영상을 쓰는 클립 중 일부만 구간이 있다. 셋 다 적거나 셋 다 뺀다 |
